...
import (
	...
	"github.com/HackathonUC2025-Hackfest/Hackfest-BE/internal/domain/clothing"
)
...
func ValidateFile(file *multipart.FileHeader) error {
	if file.Size > 1024*1024*2 {
		return clothing.ErrFileTooLarge
	}

	open, err := file.Open()
	if err != nil {
		return clothing.ErrFailedToOpenFile
	}
	defer open.Close()

	ok, fileType, err := isAllowedFileType(open)
	if err != nil {
		return clothing.ErrFailedToCheckTypeFile
	}

	if !ok {
		return clothing.ErrFileTypeNotAllowed.WithErr(errors.New("type file not allowed: " + fileType))
	}
